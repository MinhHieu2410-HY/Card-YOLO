...
CFG = {
    ...
    "data_yaml": "PlayingCards/data.yaml",
    "imgsz": 416,
    "conf": 0.25,
    "iou": 0.45,
    "device": 0,
...
st.set_page_config(page_title="Card-YOLO Demo", layout="wide")
...
def sidebar_config():
    ...
    device = st.sidebar.selectbox("Thiết bị", [0, "cpu"], format_func=lambda x: "GPU (0)" if x == 0 else "CPU")
...
def webcam_tab(model, imgsz, conf, iou, device, half):
    st.caption("Camera sẽ mở trong một cửa sổ riêng (ngoài trình duyệt). Nhấn phím 'q' trong cửa sổ đó để dừng.")
...
def main():
    st.title("Card-YOLO — Demo phát hiện lá bài")
    st.caption("Nhận diện lá bài bằng YOLOv8: kiểm tra qua ảnh/video/camera, và thêm dữ liệu huấn luyện mới.")
